...
test.describe('Battle Screen - Primary recommendation flow', () => {
  test('shows one best choice after typeahead opponent selection', async ({
    page,
  }) => {
    ...
    await test.step('See one highlighted best choice and collapsed secondary options', async () => {
      ...
      await expect(
        page.getByText('Based on Pokémon Emerald type effectiveness rules'),
      ).toHaveCount(1)
      await expect(
        page.getByRole('button', { name: /Show other options/i }),
      ).toHaveCount(1)
    ...
    await test.step('Expand other options and verify grouped sections', async () => {
      ...
      await expect(
        page.getByRole('heading', { name: /Also Good|Neutral|Risky/ }).first(),
      ).toBeVisible()
    })
  })
})
